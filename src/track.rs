...
impl Permissions {
    pub fn promote(&self) -> Self {
        let mut new_perms = *self;
        if self.contains(Permissions::READ) {
            new_perms.insert(Permissions::WRITE);
        } else {
            new_perms.insert(Permissions::READ);
        }
        new_perms
    }
...
impl Block {
    ...
    pub fn pages(&self) -> Vec<Self> {
        let page_size = crate::page_size();
        let mut pages = Vec::new();
        let mut start = align_down_to_page_size(self.ptr as usize, page_size);
    ...
    pub fn change_permissions(&self, new_permissions: Permissions) {
        ...
        debug!("Changing permissions for {:p} to {:?}", self.ptr, new_permissions.bits());
        unsafe {
            let page_size = crate::page_size();
            let start = align_down_to_page_size(self.ptr as usize, page_size);
            let end = align_up_to_page_size(self.ptr as usize + self.size_in_bytes, page_size);
            let size = end - start;
            debug!("Changing permissions for {:p} to {:?} (size: {})", self.ptr, new_permissions, size);

            let ptr = libc::mprotect(start as *mut c_void, size, new_permissions.bits() as i32);
            if ptr != 0 {
                panic!("Failed to change permissions");
            }
            debug!("Changed permissions for 0x{:08x} to {:?}", start, new_permissions.bits());
        }
    }
}
...
pub struct Track<const N: usize> {
    allocations: IndexMap<*const u8, Block>,
    // allocations: IndexMap<*const u8, Block, N>,
    // permissions: IndexMap<*const u8, Permissions, N>,
}
...
impl<const N: usize> Track<N> {
    ...
    pub fn insert(&mut self, value: Block) -> Result<bool, Block> {
        ...
        //     Err(value)
        // }
        trace!("Inserting block: {:?}", value);

        if let Some(val) = self.allocations.insert(value.ptr, value) {
            Ok(true)
        } else {
            Ok(false)
        }
...
impl<const N: usize> Debug for Track<N> {
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        f.debug_map().entries(self.allocations.iter()).finish()
    }
}
